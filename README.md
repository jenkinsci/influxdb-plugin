...
- InfluxDB v1.8 and higher (See [Breaking Changes](#breaking-changes) for details.)
- InfluxDB v2.X
- InfluxDB v3.X
...
> :warning:
>   - When you are using InfluxDB 1.x and 3.x targets, **leave the Organization blank**.
>   - InfluxDB 2.x uses organizations and buckets instead of databases and retention policies,
>   - so `organization` field is used as an identifier whether the target is InfluxDB 1.x or 2.x.
>   - InfluxDB 3.x uses databases again instead of buckets.
...
- InfluxDB 3.x supports only `Secret Text` for token authentication.

## Usage

### Global Listener

When `globalListener` is set to `true` for a target in which no results were published during the build, it will
automatically publish the result for this target when the build is completed.

To configure the global listener, you can use environment variables prefixed with `INFLUXDB_PLUGIN`. The following
variables are supported and all correspond to an `influxDbPublisher` optional parameter.
...
**_NOTE:_** The environment variables must be set on the final build object. If you are creating or updating these
variables in a pipeline, you should make sure they are exported with
an [EnvironmentContributingAction](https://javadoc.jenkins.io/hudson/model/EnvironmentContributingAction.html).
...
This adds the fields `field_a` and `field_b` with values `11` and `12` respectively to a measurement called
`jenkins_custom_data`.
...
please use
the [Jenkins issue tracker](https://issues.jenkins-ci.org/issues/?jql=project%20%3D%20JENKINS%20AND%20component%20%3D%20influxdb-plugin).
